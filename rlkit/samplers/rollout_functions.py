...
def rollout_with_prev_action(
    ...
    prev_a = np.zeros(env.action_space.shape[0])
    ...
    if len(prev_actions.shape) == 1:
        prev_actions = np.expand_dims(prev_actions, 1)
...
def rollout_with_obs_history(
    ...
    o_ = env.reset()
    obs_list.append(o_)
    o = np.zeros(obs_dim * stacksize)
    o[ - len(obs_list) *  obs_dim : ] = np.concatenate(obs_list)
    ...
    while path_length < max_path_length:
        ...
        next_o_, r, done, env_info = env.step(copy.deepcopy(a))

        obs_list.append(next_o_)

        if len(obs_list) < stacksize:
            next_o_ = np.concatenate(obs_list)
            next_o = np.zeros(obs_dim * stacksize)
            next_o[-(len(obs_list)) * obs_dim:] = next_o_
        
        else:
            next_o = np.concatenate(obs_list[-stacksize:])
