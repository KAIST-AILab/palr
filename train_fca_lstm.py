...
from imitation.fca import FCA, FCA_MINE, FCA_MINE_LSTM
...
def train(configs):
    ...
    if 'FCA_LSTM_MINE' in configs['algorithm']:
        ...
        fca_trainer.train(total_iteration=configs['total_iteration'], discriminator_steps=configs['discriminator_steps'])
...
if __name__ == "__main__":
    ...
    stacksizelist = [3]                                     # MDP=0
    seedlist = [0]                              #,3,4]
    ib_coef_list = [1e-3]
    mine_loss_coef_list = [1e-4]
    mine_steps_list = [10]
    ...
    envtype, seed, ib_coef, min_loss_coef, mine_steps = \
        list(product(envlist, seedlist, ib_coef_list, mine_loss_coef_list, mine_steps_list))[pid]
    ...
    if ib_coef > 0:
        if min_loss_coef > 0:
            algorithm = f'FCA_LSTM_MINEREG_W{stacksize}_IB_MSTEP{mine_steps}_{batch_size}'
        else:
            algorithm = f'FCA_W{stacksize}_IB_{batch_size}'
        
    else:
        if min_loss_coef > 0:
            algorithm = f'FCA_LSTM_MINEREG_W{stacksize}_MSTEP{mine_steps}_{batch_size}'
        else:
            algorithm = f'FCA_W{stacksize}_{batch_size}'
